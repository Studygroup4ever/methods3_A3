...
demo %>% filter(Diagnosis == "Control") 
library(tidyverse)

danish <- pitch_data %>% filter(Study < 5)
danish %>% group_by(Diagnosis) %>%  summarise(age_count=n())

danish$ID <- 1:264

arti <- read.delim("Articulation.txt", sep =",")
...
read_pitch <- function(filename) {
    ...
    Subject <-
        str_extract(filename, 'S(\\d\\d\\d|\\d\\d)') %>% 
...
colnames(pitch_data) <- c('soundname', 'Filename',
                          'Study',
                          'Diagnosis',
                          'Subject',
                          'Trial',
...
colnames(pitch_data)[5] <- "Participant"
...
dempitch <- merge(Arti_data, pitch_data, by = 'soundname') 
sounddata <- merge(dempitch, Dem_data, by = 'Participant')

sounddata$Study.x ==sounddata$Study.y 
...
sounddata <-  sounddata %>% filter(Study.x < 5)

summary(mean(sounddata$SANS))
...
```{r}
sounddata <- sounddata %>%  na.omit()
write.csv(sounddata, "sounddata.csv")

sounddata %>% group_by(Study.x) %>%  summarise(gender_count = n())
```
...
sounddata$pause_dur <-  (sounddata$dur..s.-sounddata$phonationtime..s.)/sounddata$npause
sounddata$pause_dur <- ifelse(sounddata$npause == 0,NA,sounddata$pause_dur)

sounddata$pause_dur <- as.numeric(sounddata$pause_dur)

sounddata <- sounddata %>% 
    mutate(IQR_scale = scale(IQR)) %>% 
    mutate(phonationtime_scale = scale(phonationtime..s.)) %>% 
    mutate(speechrate_scale = scale(speechrate..nsyll.dur.)) %>% 
    mutate(pause_dur_scale = scale(pause_dur))

pacman::p_load(lme4)

sounddata$Diagnosis.x <- as.factor(sounddata$Diagnosis.x)

m1 <- glmer(Diagnosis.x ~IQR_scale + (1|Participant),data=sounddata, family =binomial)

m2 <- glmer(Diagnosis.x ~ phonationtime_scale+ (1|Participant),data = sounddata, family = binomial )
#speech rate
m3 <- glmer(Diagnosis.x ~ speechrate_scale+ (1|Participant),data=sounddata, family = binomial)
#pause dur
m4 <- glmer(Diagnosis.x ~ pause_dur_scale+ (1|Participant), data = sounddata,family = binomial)




m5 <- glmer(Diagnosis.x ~ 0 + Study.x + Study.x:Diagnosis.x + (1|Participant), data=sounddata, family = binomial)
