...
p_load(purrr, tidyverse, reshape2, lme4, car)
```
...
df %>% group_by(Study.y, Diagnosis.x) %>% summarize(n())

df %>% group_by(Gender, Diagnosis.x) %>% summarize(n())

df %>% group_by(Study.y) %>% summarize(n())

df %>% group_by(Trial) %>% summarize(n())

df %>% select(c(Study.y, Diagnosis.x, SANS)) %>% na.omit() %>% group_by(Study.y, Diagnosis.x) %>% summarize(mean(SANS))
...
```{r}
#df <- read.csv('sounddata.csv')
df <- df %>% 
    mutate(pause_dur = (dur..s. - phonationtime..s.)/npause)

df$pause_dur <- ifelse(df$pause_dur == Inf, 0, df$pause_dur) %>% as.numeric()
...
df_sub <- df %>% select(c(Diagnosis.x, pause_dur_scaled, Participant)) %>% na.omit()
```
...
m2 <- glm(data = df, Diagnosis.x ~ ProportionSpokenTime_scaled, family = 'binomial')
summary(m2)

m3 <- glm(data = df, Diagnosis.x ~ SpeechRate_scaled, family = 'binomial')
summary(m3)

m4 <- glm(data = df, Diagnosis.x ~ pause_dur_scaled, family = 'binomial')
summary(m4)

pitch_vari_model <- glmer(data = df, 
                          Diagnosis.x ~ IQR_scaled + (1 | Participant), 
                          family = 'binomial')
summary(pitch_vari_model)

SpokenTime_model <- glmer(data = df, 
                          Diagnosis.x ~ ProportionSpokenTime_scaled + (1 | Participant), 
                          family = 'binomial')
summary(SpokenTime_model)

SpeechRate_model <- glmer(data = df, Diagnosis.x ~ SpeechRate_scaled + (1 | Participant), 
                          family = 'binomial')
summary(SpokenTime_model)

pause_dur_model <- glmer(data = df, Diagnosis.x ~ pause_dur_scaled  + (1 | Participant), 
                          family = 'binomial')
