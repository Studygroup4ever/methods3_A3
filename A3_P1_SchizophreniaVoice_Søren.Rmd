...
trial <- read.delim('Pitch/Study1D0S101T1_f0.txt', sep = '\t', header = T)

# IQR(trial$f0)
# 
# f_name <- 'Study1D0S10T12T13_f0.txt'
# 
# str_extract(f_name, 'S(\\d\\d\\d|\\d\\d)')
# 
# pos <- str_locate(f_name, 'T')[1]
# 
# strsplit(f_name, '')
# 
# length(split[[1]])
# 
# substr(f_name, pos, length(split[[1]]))
...
Subject <- str_extract("Study1D0S101T1_f0.txt", 'S(\\d\\d\\d | \\d\\d)')
