...
Study6D0S001T1 participant 001
```{r}
...
Check if study is the same
```{r}
unique(sounddata$Study.x == sounddata$Study.y) #TRUE
```

```{r}
# make variables we will need - move this down
# constructing pause duration 
sounddata <- sounddata %>% 
    mutate(pause_dur = (dur..s.- phonationtime..s.)/npause)

sounddata$pause_dur[sounddata$pause_dur == 0] <- NA
...
sounddata <- sounddata %>% 
    mutate(scale_pause_dur = scale(pause_dur),
           ...
           scale_IQR = scale(IQR),
           scale_speechrate_syll_dur = scale(speechrate..nsyll.dur.)) #QUESTION: is it syllables pr SPOKEN second or just second in general
