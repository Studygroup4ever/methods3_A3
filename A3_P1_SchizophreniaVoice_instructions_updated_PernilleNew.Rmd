...
# count participants
sounddata %>% group_by(Gender) %>% summarise(mean(Age)) #F = 21.8, M = 23.8

sub <- sounddata %>% subset(Diagnosis.y =="Schizophrenia")
sounddata$Diagnosis.y <- as.factor(sounddata$Diagnosis.y)
levels(sounddata$Diagnosis.y)

sub %>% summarise(mean(Age))
...
```{r}
unique(sounddata$Study.x == sounddata$Study.y)
```
