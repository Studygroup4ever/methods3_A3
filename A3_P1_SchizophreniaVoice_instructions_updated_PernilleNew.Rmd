...
```{r}
pacman::p_load(tidyverse, stringr,purrr,lme4,ggpubr)
...
# Sort diagnosis
Dem_data$Diagnosis <- sort(Dem_data$Diagnosis)
...
#merged_df <- merged_df %>% na.omit() #only study 1 and 2 left
...
```{r}
# Reading in Julia's so we have the same. It is na-omitted
#sounddata <- read.csv("sounddata.csv")

#Trying something else

# count participants
#sounddata %>% group_by(Diagnosis.y) %>% summarise(mean(Age)) #F = 21.8, M = 23.8
