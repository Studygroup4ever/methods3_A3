...
*a2p2 note:* 
broom.mixed allows me to plot mixed effect models
augment function

augment(Model) %>% 
ggplot(visir, color = diag, group = id)
geom_jitter(aes(y=chimlu), 
geom:_line(y =.fitted))


Notes: Loading dataset: 
- issue: control participants and skizos have the same ID, ID is not unique (repeating), so using the merging duplicates the dataset, additional step to instructions = create unique ID. 
- Either, rename the column for all values that skizos have, -s, or paste the diagnosis + ID in one column
...
DemoDanish <- subset(Demo, Language == "Danish")

DemoDanish$ID <- 1:264

Arti <- read.delim("Articulation.txt", header = TRUE, sep = ",")

read.delim("Articulation.txt", header = TRUE, sep = ",")

# filename <-  read.delim("Pitch/Study5D1S26T2.txt")
# filename <- "Study5D1S26T2.txt"

#for(n in 1:length(list){...})

#SØREN MAGIC 
# pos = str_locate(filename, 'T')[1]
# split = str_split(filename, '') # for at få længde
# split[[1]]
#substr(filename, pos, length(split[[1]])) #finder T, og tager fra T til længden af vores string
#str_extract("Study5D1S26T2.txt",'\\d\\d\)
...
read_pitch <- function(filename) {
    ...
    Subject <- str_extract(filename, 'S(\\d\\d\\d|\\d\\d)') #kig efter 3 først
    
    pos <- str_locate(filename, 'T')[1]
    split <- strsplit(filename, '') #del alting
    Trial <- substr(filename, pos, length(split[[1]]))
    ...
    data <- c(filename, Study, Diagnosis, Subject, Trial, mean, sd, median, iqr)
     return(data)
}

#iqr #difference between Q3 and Q1, dispersion
test_data <-  read_pitch("Pitch/Study1D0S101T1_f0.txt")
test_data

IQR("Pitch/Study1D0S101T10_f0.txt")
# try lapply??
```
...
```{r}
# Reading in
list <- list.files(path = "Pitch/",pattern = "*.txt", full.names = T)

list

lille = read.table(list, header = T)
...
```{r}
#Det jeg fik fra gruppen
list <- list.files(path = "Pitch/",pattern = "*.txt")
path <- "Pitch/"
data_lists <- as.data.frame(NULL)

for (file in 1:length(list)){
    df <- read.delim(paste0(path, list[file]))
    df$Study <- ""
    df$Study <- str_extract(list[file], "\\d {6}") #for letters [a-z]
    data_lists <- rbind(data_lists, df)
}


# test it on just one file while writing the function
test_data = read_pitch("Pitch/Study5D1S26T2.txt")



# when you've created a function that works, you can
pitch_data = list.files(path = "Desktop/EM3/methods3_A3/Pitch/",pattern = ".txt") %>% ## NB replace with your path to the files
