...
```{r}
trial <- read.delim("Pitch/Study6D1S044T9.txt", sep = "\t", header =T)

arti <- read.delim("Articulation.txt", sep = ",", header =T)

file1 <- "Study6D1S044T9.txt"
file2 <- "Study6D1S203T9.txt"

str_extract(file1, "(S\\d\\d\\d|\\d\\d)")
str_extract(file2, "S(\\d\\d\\d|\\d\\d)")
...
read_pitch <- function(filename) {
    ...
    data <- c(filename, Study, Diagnosis, Subject, Trial, mean, sd, iqr, median)
    
    return(data)
}

# test it on just one file while writing the function
test_data = read_pitch("Pitch/Study6D1S044T9.txt")
